...
public static class PurchaseOrderEndpoints
{
    public static IEndpointRouteBuilder MapPurchaseOrderEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        {
            var detail = await mediator.Send(new GetPurchaseOrderByIdQuery(RouteParams.Decode(doku)), ct);
            if (detail is null)
            {
        ...
        group.MapDelete("/{*doku}", async (string doku, HttpContext ctx, IMediator mediator, CancellationToken ct) =>
        {
            ...
            {
                await mediator.Send(
                    new DeletePurchaseOrderCommand(RouteParams.Decode(doku), ifMatch), ct);
