...
namespace GalvaERP.Features.PurchaseRequisitions;

public static class PurchaseRequisitionEndpoints
{
    public static IEndpointRouteBuilder MapPurchaseRequisitionEndpoints(this IEndpointRouteBuilder app)
    {
        var group = app.MapGroup("/api/purchase-requisitions").WithTags("PurchaseRequisitions").WithOpenApi();

        group.MapGet("/", async (IMediator mediator, CancellationToken ct) =>
        {
            var list = await mediator.Send(new GetPurchaseRequisitionsQuery(), ct);
        ...
        group.MapPost("/", async (CreatePurchaseRequisitionCommand command, IMediator mediator, CancellationToken ct) =>
        {
        ...
        group.MapPut("/{*doku}", async (string doku, HttpContext ctx, IMediator mediator, CancellationToken ct) =>
        {
            ...
            UpdatePurchaseRequisitionCommand body;
            try
            {
                body = await ctx.Request.ReadFromJsonAsync<UpdatePurchaseRequisitionCommand>(cancellationToken: ct)
            ...
            catch (Common.Exceptions.ConcurrencyException ex)
            {
